"""
Ask TIM Backend v6.0.14.5-fixed
- Fixes SyntaxError line 116
- Guardrails + auto-country + versatile model
"""
...
def check_guardrails(question, headers, ip):
    ...
    if not is_on_topic and not persist and len(question) > 5:
...
def call_groq(sys_prompt, user_q, ctx=""):
    # FIXED: proper string construction, no unterminated literal
...
def ask():
    ...
    return jsonify({"answer": answer, "version":"6.0.14.5-fixed", "detected_country": country})
